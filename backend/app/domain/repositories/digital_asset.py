...
class DigitalAssetRepository(BaseRepository["DigitalAsset"]):
    ...
    async def get_by_hash(self, sha256_hash: str) -> DigitalAsset:
        ...
        Content-addressed lookup against the sha256_hash column (global
        partial unique identity). Used by the upload pipeline (E5.T4) to
        ...
        Example:
            ...
            # Retrieves the content-addressed asset with matching checksum
            ```
    ...
    async def get_by_normalized_value(
        ...
        The domain identity is unique per user: different users can have
        the same normalized_value + asset_type pair (they reference the
        same threat/resource). File-like assets dedup globally by
        sha256_hash — use get_by_hash() for those.
