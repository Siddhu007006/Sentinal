...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=False,
            comment="Asset owner; foreign key to users.id with RESTRICT on delete",
        ),
        sa.Column(
            "upload_id",
            sa.UUID(as_uuid=True),
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            name="fk_digital_assets_user_id_users",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["upload_id"],
            ["uploads.id"],
            name="fk_digital_assets_upload_id_uploads",
            ondelete="SET NULL",
        ),
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_digital_assets_normalized_value_type", table_name="digital_assets"
    )
