...
def upgrade() -> None:
    ...
    op.create_table(
        'uploads',
        sa.Column('user_id', sa.UUID(as_uuid=True), nullable=False, comment='Foreign key to users table; every upload belongs to exactly one user'),
        sa.Column('original_filename', sa.String(255), nullable=False, comment='Original filename as provided by user (normalized, may contain special chars)'),
        sa.Column('storage_key', sa.String(1024), nullable=False, comment='S3/MinIO object key (immutable, unique, max 1024 chars)', unique=True, index=True),
        sa.Column('content_type', sa.String(100), nullable=False, comment='MIME type (e.g., application/pdf, image/png)'),
        sa.Column('file_size_bytes', sa.BigInteger(), nullable=False, comment='File size in bytes (>= 0)'),
        sa.Column('checksum_sha256', sa.String(64), nullable=True, comment='SHA-256 hash of file (64-char hex); null until validation completes'),
        sa.Column('upload_status', sa.String(20), nullable=False, server_default="'pending'", comment='Upload lifecycle state: pending, processing, completed, failed'),
        sa.Column('completed_at', sa.TIMESTAMP(timezone=True), nullable=True, comment='When upload reached terminal state (completed/failed); null if pending/processing'),
        ...
        sa.CheckConstraint("upload_status IN ('pending', 'processing', 'completed', 'failed')", name='ck_uploads_upload_status_valid'),
        sa.CheckConstraint('file_size_bytes >= 0', name='ck_uploads_file_size_bytes_nonnegative'),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='fk_uploads_user_id_users'),
